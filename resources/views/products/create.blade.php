...
<div class="rounded-lg border-4 border-green-500 my-2 py-2 px-2">
    <form action="{{route('products.store')}}" method="POST" class="mt-5" enctype="multipart/form-data">
        ...
        <input type="text" placeholder="Product Name" name="name" class="w-full rounded-lg border-2 border-red-300 my-2" value="{{old('name')}}">
        @error('name')
            <p class="text-red-400 text-xs -mt-2">{{$message}}</p>
        @enderror
        ...
        <input type="text" placeholder="Brand" name="brand" class="w-full rounded-lg border-2 border-red-300 my-2" value="{{old('brand')}}">
        @error('brand')
            <p class="text-red-400 text-xs -mt-2">{{$message}}</p>
        @enderror

        <textarea placeholder="Desription" rows="8" name="description" class="w-full rounded-lg border-2 border-red-300 my-2" value="">{{old('description')}}</textarea>
        @error('description')
            <p class="text-red-400 text-xs -mt-2">{{$message}}</p>
        @enderror
